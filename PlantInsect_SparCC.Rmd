...
```{r}
insect_gab<-g_abundance(layer_mat = Insectmat, g = insect_sparCC)
plant_gab<-g_abundance(layer_mat = Plantmat, g = plant_sparCC)
plot(insect_gab, vertex.label.color="black",
      vertex.color = vertex.attributes(insect_gab)$rel_ab, vertex.label.cex=.5,
      vertex.label.dist=1,layout=layout_with_kk, vertex.size = 5)
plot(plant_gab, vertex.label.color="black",
      vertex.color = vertex.attributes(plant_gab)$rel_ab, vertex.label.cex=.5,
      vertex.label.dist=1,layout=layout_with_kk, vertex.size = 5)
```
...
vertex.attributes(insect_sparCC)$Group[1:12]
```
...
vertex.attributes(insect_sparCC)$Group[by_degree]
by_betweenness<-which(vertex.attributes(bt[[1]])$hl == Redpal[1])
vertex.attributes(insect_sparCC)$Group[by_betweenness]
by_closeness<-which(vertex.attributes(cl[[1]])$hl == Redpal[1])
vertex.attributes(insect_sparCC)$Group[by_closeness]
```
