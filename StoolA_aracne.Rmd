...
hist(degree.distribution(T1_aracne), freq = FALSE, main = "Histograma y densidad",
     ylab = "Densidad")

# Calculamos la densidad
dx <- density(degree.distribution(T1_aracne), bw = 0.08)

# Añadimos la línea de densidad
lines(dx, lwd = 2, col = "red")

# Añadimos los datos con riudo en el eje X
rug(jitter(degree.distribution(T1_aracne)))
```
...
plot(T1_cluster, T1_aracne, vertex.label.color="black",
     vertex.label.cex=.5,layout=layout_with_kk, vertex.size = 5)
T2_cluster<-cluster_louvain(T2_aracne)
plot(T2_cluster, T2_aracne, vertex.label.color="black",
     vertex.label.cex=.5,layout=layout_with_kk, vertex.size = 5)
T3_cluster<-cluster_louvain(T3_aracne)
plot(T3_cluster, T3_aracne, vertex.label.color="black",
     vertex.label.cex=.5,layout=layout_with_kk, vertex.size = 5)
```
...
hl3<-as.vector(vertex.attributes(g.list_cl[[3]])$hl)
hl_colors<-c(hl1, hl2, hl3)
Nodes<-length(V(g.list[[1]]))
Layers<-length(g.list)
node.colors.matrix <- matrix(hl_colors, Nodes, Layers)
