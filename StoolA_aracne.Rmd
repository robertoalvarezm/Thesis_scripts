...
ggplot_df<-ggplot_df[-which(ggplot_df$Sums < 300),]
library(ggplot2)
ggplot(ggplot_df)+
...
```{r}
clust<-list()
for(i in 1:length(unique(ml_clust$cid))){
  clust[[i]]<-which(unique(ml_clust$cid)[i] == ml_clust$cid)
}
clust2<-list()
...
```{r}
net_times<-list()
net_times[2] <- erdos.renyi.game(10, 0.3)
net_times
```
